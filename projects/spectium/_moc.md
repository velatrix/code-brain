...
**In progress: Access Control** (repo `ACCESS-CONTROL.md`, phases AC0–AC7, on `feat/access-control` since 2026-09-26; not merged, not pushed). One permission model, enforced on every endpoint, with a test for every endpoint — started after an audit of all 157 HTTP actions and 15 hub methods found four chains that crossed tenants (among them a live device session any user could drive, a public default signing key, and removal that left project access in place). AC0–AC3 are done: AC1 contained those chains; AC2 built an HTTP harness over the real backend and a policy matrix of every endpoint against every kind of caller (1562 cells); AC3 made every endpoint and hub method declare its requirement, enforced before model binding by [[vappcore|VAppCore]] 3.0.0 and answered by one rule read from the database — the backend refuses to start if any endpoint is undeclared, and refusals are 404 for what the caller may not see, 403 naming the missing permission, 401 only without credentials. Verified: integration 2085/2085. The repo's local `main` is at `90e22b1` (AC1 + M6), unpushed. Next, after the user's go: AC4, the model itself (org Owners and Admins reach every project, grant ceilings, offboarding transfers, a real system administrator), then credentials (AC5), the UI (AC6) and the docs walk (AC7). The reusable lessons are in the 2026-09-26 [[log]] entry.
...
- [[vappcore]] — the backend builds on it (MVC filters, VQueryParser pagination/filtering); the access-control branch also enforces every endpoint through its 3.0.0 scoped authorization
